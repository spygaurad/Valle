...
class Img2Img_Txt2Img_Model(nn.Module):
    def __init__(self, char_model, config, device):
        super(Img2Img_Txt2Img_Model, self).__init__()
    ...
    def forward(self, batch_output):
        ...
        return img2img_dec_img, txt2img_dec_img
...
class Img2Img_Txt2Img(Task):
    ...
    def build_model(self):
        return Img2Img_Txt2Img_Model(self.char_model, self.config, self.device)
        ...
        for epoch in range(self.current_epoch, self.config.epoch + 1):
            ...
            with progressbar.ProgressBar(max_value=len(self.train_set)) as bar:
                ...
                for index, batch_output in enumerate(self.train_set):
                    ...
                    img2img_dec_img, txt2img_dec_img = self.model(batch_output)
                    ...
                    loss = img2img_loss + txt2img_loss
                    loss.backward()
            ...
            total_loss = total_img2img_loss + total_txt2img_loss
